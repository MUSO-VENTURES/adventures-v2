...
-- Postgres primary-key columns can never be nullable, so the key has to
-- move to a plain surrogate id, with the same coalesce()'d uniqueness
-- taking over the "one progress row per party+adventure(+freeplay)+mode"
-- invariant the old composite PK enforced.
