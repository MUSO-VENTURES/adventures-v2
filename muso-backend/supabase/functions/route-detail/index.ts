...
Deno.serve(async (req) => {
  ...
  const { data: stops, error: stopsErr } = await client
    .from("route_stops")
    .select("id, stop_order, name, description, emoji, is_mystery, venue_id, offered_choices, venues(lat, lng, address, muso_rating, muso_rating_count, image_url, hours)")
    .eq("route_id", routeId)
  ...
  const itinerary = visibleStops.map((s: Record<string, unknown>) => {
    const venue = s.venues as { lat: number | null; lng: number | null; address: string | null; muso_rating: number | null; muso_rating_count: number | null; image_url: string | null; hours: unknown | null } | null;
